...
# Method to fire API request to GPT-Neo for recommendation
...
# Method to prepare parameters and few-short learning input for GPT-Neo
...
def read_pdf():
    ...
    reqs_doc = fitz.open(latest_file)
    reqs_doc_content = ""
    for page in reqs_doc:
        reqs_doc_content += page.get_text()
    input_pdf_file_name = latest_file[6:len(latest_file)-4]
    return reqs_doc_content, input_pdf_file_name
...
def annotate_pdf(detected, input_pdf_file_name):
    ...
    for pg, page in enumerate(reqs_doc):
        for key, value in detected.items():
            if key=='Rec':
    ...
    annotated_pdf = 'annotated_docs\\'+input_pdf_file_name+'__annotated_'+datetime.now().strftime("%Y-%m-%d_%H-%M-%S")+'.pdf'
